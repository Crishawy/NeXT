...
flags.DEFINE_string('decay_type', 'exponential', 'The lr decay type')
flags.DEFINE_string('shape', 'vase', 'scene of deepvoxel')
...
def main(unused_argv):
    ...
    if config.dataset_loader == 'deepvoxel':
        config.shape = FLAGS.shape
    ...
    if config.dataset_loader == 'deepvoxel':
        config.near = dataset.near
        config.far = dataset.far
    ...
    if FLAGS.decay_type == 'exponential':
        ...
        learning_rate_fn = functools.partial(
            math.transformer_learning_rate_decay,
            total_steps=config.max_steps,
            base=config.lr_init,
            decay_type=FLAGS.decay_type,
            warmup_steps=config.lr_delay_steps,
            warmup_init_lr=config.warmup_init_lr,
            lr_min=config.lr_final,
            linear_end=config.lr_final
        )

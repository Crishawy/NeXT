...
from internal.nerf_transformer import get_nerf_transformer
...
class MipNerfModel(nn.Module):
    ...
    def __call__(self, rng, rays, randomized, white_bkgd):
        ...
        if self.model == 'mlp':
            model = MLP()
        elif 'next' in self.model:
            model = get_nerf_transformer(self.model)
        else:
            raise NotImplementedError(f'Not implement {self.model}')
        ...
        for i_level in range(self.num_levels):
            ...
            if self.use_viewdirs:
                ...
                raw_rgb, raw_density = model(samples_enc, viewdirs_enc)
            else:
                raw_rgb, raw_density = model(samples_enc)

...
def train_step(model, rng, state, batch, lr):
    ...
    def loss_fn(variables):
        rays = batch["rays"]
        ret = model.apply(variables, key_0, key_1, rays, FLAGS.randomized, train=True)
...
def main(unused_argv):
    ...
    if FLAGS.dataset == 'deepvoxel':
        FLAGS.near = dataset.near
        FLAGS.far = dataset.far
    ...
    if FLAGS.decay_type == 'exponential':
        ...
        learning_rate_fn = functools.partial(
            utils.transformer_learning_rate_decay,
            total_steps=FLAGS.max_steps,
            base=FLAGS.lr_init,
            decay_type=FLAGS.decay_type,
            warmup_steps=FLAGS.lr_delay_steps,
            warmup_init_lr=FLAGS.warmup_init_lr,
            lr_min=FLAGS.lr_final,
            linear_end=FLAGS.lr_final
        )
    ...
    def render_fn(variables, key_0, key_1, rays):
        ...
        return jax.lax.all_gather(
            model.apply(variables, key_0, key_1, rays, FLAGS.randomized, False),
            axis_name="batch")

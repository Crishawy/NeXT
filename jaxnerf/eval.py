...
def main(unused_argv):
    ...
    def render_fn(variables, key_0, key_1, rays):
        return jax.lax.all_gather(
            model.apply(variables, key_0, key_1, rays, False, False), axis_name="batch")
    ...
    for step in list(map(int, FLAGS.ckpt_step.split(','))):
        ...
        print(f'Checkpoint {step} avg PSNR: ', np.mean(np.array(psnr_values)))
        print(f'Checkpoint {step} avg SSIM: ', np.mean(np.array(ssim_values)))

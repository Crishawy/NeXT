...
def define_flags():
    ...
    # deepvoxel dataset
    flags.DEFINE_string('shape', 'cube', 'scene for deep voxel')
    flags.DEFINE_integer('test_skip', 8, 'skip test for large scenes')

    # checkpoint step
    flags.DEFINE_string('ckpt_step', '1000000', 'checkpoint load step')

    # lr scheduler
    flags.DEFINE_string('decay_type', 'cosine', 'lr scheduler type')
    flags.DEFINE_float('warmup_init_lr', 1e-6, 'warmup initial learning rate')

    # nerf transformer
    flags.DEFINE_string('backbone', 'next_s', 'backbone of nerf transformer')
    flags.DEFINE_integer('shift_size', 0, 'the window shift size')
    flags.DEFINE_integer('window_size', 0, 'local window size')
    flags.DEFINE_integer('embed_dim', 256, 'transformer embedding dim')
    flags.DEFINE_integer('depth', 2, 'network depth')
    flags.DEFINE_integer('output_c', 3, 'output channel')
    flags.DEFINE_integer('num_heads', 8, 'number of attention heads')
    flags.DEFINE_float('mlp_ratio', 1., 'mlp ratio')
    flags.DEFINE_float('drop_rate', 0., 'dropout rate')
    flags.DEFINE_float('attn_drop_rate', 0., 'attention dropout rate')
    flags.DEFINE_float('drop_path_rate', 0., 'drop path rate')
    flags.DEFINE_string('skips', '0,1', 'skip layers')

    flags.DEFINE_string("train_dir", './exp/debug', "where to store ckpts and logs")
    flags.DEFINE_string("data_dir", './data/nerf_synthetic/chair', "input data directory.")
...
def transformer_learning_rate_decay(step,
                                    total_steps,
                                    base,
                                    decay_type,
                                    warmup_steps,
    ...
    if step < warmup_steps:
        lr = warmup_init_lr + step * (base - warmup_init_lr) / warmup_steps
        return lr
    ...
    if decay_type == 'linear':
        ...
        lr = lr_min + (base - lr_min) * 0.5 * (1. + np.cos(np.pi * progress))
    else:
...
def learning_rate_decay(step,
    ...
    # warmup
    if lr_delay_steps > 0:
        # A kind of reverse cosine decay. warm up
